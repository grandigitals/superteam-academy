...
CREATE POLICY IF NOT EXISTS "Users can insert own row" ON users 
  FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Users can update own row" ON users 
  FOR UPDATE USING (true);

-- FIX 3: Add INSERT and UPDATE policies for enrollments table
CREATE POLICY IF NOT EXISTS "Enrollments insert own" ON enrollments 
  FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Enrollments update own" ON enrollments 
  FOR UPDATE USING (true);

-- FIX 4: Add INSERT policy for lesson_completions
CREATE POLICY IF NOT EXISTS "Lesson completions insert own" ON lesson_completions 
  FOR INSERT WITH CHECK (true);

-- FIX 5: Add INSERT policy for xp_ledger
CREATE POLICY IF NOT EXISTS "XP ledger insert own" ON xp_ledger 
  FOR INSERT WITH CHECK (true);

-- FIX 6: Add INSERT and UPDATE policies for streaks
CREATE POLICY IF NOT EXISTS "Streaks insert own" ON streaks 
  FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Streaks update own" ON streaks 
  FOR UPDATE USING (true);

-- FIX 7: Add INSERT policy for credentials
CREATE POLICY IF NOT EXISTS "Credentials insert own" ON credentials 
  FOR INSERT WITH CHECK (true);
